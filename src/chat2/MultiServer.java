...
public class MultiServer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(socket.getInetAddress()+"의" + socket.getPort() + "포트에" 
				+ socket.getLocalAddress()+"의" + socket.getLocalPort()+"포트로 연결되었습니다.");
			
			//메시지를 보낼준비(서버->클라이언트)
			out = new PrintWriter(socket.getOutputStream(), true);
			//메시지를 읽을(받을)준비(클라이언트->서버)
			in = new BufferedReader(new InputStreamReader(socket.getInputStream()));				
			
			/*
			클라이언트가 서버로 전송하는 최초의 메시지는 "대화명"이므로 
			메시지를 읽은후 변수에 저장하고 클라이언트쪽으로 Echo해준다.
			 */ 
			if(in != null) {
			...
			while(in != null) {
				...
				System.out.println(name + " ==> " + s);//서버의 콘솔출력
